			...
			actions : {
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "SalesOrderCollection",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("SalesOrderKey") === oObject.SalesOrderKey;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].SalesOrderKey;
									} else {
										sCurrentId = aItemsNotInTheList[0].SalesOrderKey;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("SalesOrderCollection", {
										SalesOrderKey : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("SalesOrderKey"),
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<SalesOrderCollection> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnUnitNumber : function () {
					return this.theListShouldBeSortedAscendingOnField("TotalSum");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all SalesOrderCollection to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<SalesOrderCollection> (" + iExpectedLength + ")"}),
								success : function () {
